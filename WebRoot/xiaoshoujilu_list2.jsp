...
<html>
  <head>
    ...
    <title>出货记录</title><LINK href="CSS.css" type=text/css 
rel=stylesheet>
...
<table width="100%" border="1" align="center" cellpadding="3" cellspacing="1" bordercolor="00FFFF" style="border-collapse:collapse">  
  <tr>
    ...
    <td bgcolor='#CCFFFF'>类药品别</td>
    <td bgcolor='#CCFFFF'>价格</td><td bgcolor='#CCFFFF'>库存</td><td bgcolor='#CCFFFF'>客户姓名</td><td bgcolor='#CCFFFF'>出货数量</td><td bgcolor='#CCFFFF'>备注</td><td bgcolor='#CCFFFF'>经手人</td>
  ...
  int curpage=1;//当前页
				...
				String fysql="select count(id) as ss from xiaoshoujilu where jingshouren='"+request.getSession().getAttribute("username")+"'";
...
if(((curpage-1)*page_record)==0 || curpage==0)
   {
  sql="select  * from xiaoshoujilu where jingshouren='"+request.getSession().getAttribute("username")+"' ";
   }
   else
   {
   String tempsql="select  id from xiaoshoujilu where jingshouren='"+request.getSession().getAttribute("username")+"' order by id desc limit 0,"+(curpage-1)*page_record+"";
  ...
  sql="select  * from xiaoshoujilu where id not in ("+tempidstring+") and jingshouren='"+request.getSession().getAttribute("username")+"' ";
  }
...
%>
  <tr>
    ...
    <td><%=bianhao %></td><td><%=mingcheng %></td><td><%=leibie %></td><td><%=jiage %></td><td><%=kucun %></td><td><%=kehuxingming %></td><td><%=xiaoshoushuliang %></td><td><%=beizhu %></td><td><%=jingshouren %></td>
    <td width="138" align="center"><%=addtime %></td>
    <td width="60" align="center"><a href="xiaoshoujilu_updt.jsp?id=<%=id%>">修改</a>  <a href="del.jsp?id=<%=id %>&tablename=xiaoshoujilu" onClick="return confirm('真的要删除？')">删除</a></td>
  </tr>
  	<%
  }
   %>
...
<p align="center">&nbsp;共<%=zgs%>条记录&nbsp;&nbsp;<%=page_record %>条/页　<a href="xiaoshoujilu_list2.jsp?page=1">首页</a>　<a href="xiaoshoujilu_list2.jsp?page=<%= curpage-1%>">上一页</a>　<A href="xiaoshoujilu_list2.jsp?page=<%= curpage+1%>">下一页</A>　<a href="xiaoshoujilu_list2.jsp?page=<%=zys %>">尾页</A>　当前第<FONT color=red><%=curpage %></FONT>页/共<FONT color=red><%=zys %></FONT>页</p>
  </body>
</html>
