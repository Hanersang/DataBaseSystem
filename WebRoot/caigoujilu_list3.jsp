...
%>
  ...
  &nbsp;编号：
  <input name="bianhao" type="text" id="bianhao" style='border:solid 1px #000000; color:#666666' size="12" />
  &nbsp;&nbsp;药品名称：
  ...
  供应商：<select name='gongyingshang' id='gongyingshang'><option value="">所有</option><% sql="select mingcheng from gongyingshangxinxi order by id desc";RS_result=connDbBean.executeQuery(sql);while(RS_result.next()){%><option value="<%= RS_result.getString("mingcheng")%>" ><%=RS_result.getString("mingcheng")%></option><%}%></select>&nbsp;&nbsp;经手人：<input name="jingshouren" type="text" id="jingshouren" style='border:solid 1px #000000; color:#666666' size="10" />
...
<table width="100%" border="1" align="center" cellpadding="3" cellspacing="1" bordercolor="00FFFF" style="border-collapse:collapse">  
  <tr>
    ...
    <td bgcolor='#CCFFFF'>库存</td><td bgcolor='#CCFFFF'>进货数量</td><td bgcolor='#CCFFFF'>进货价</td><td bgcolor='#CCFFFF'>供应商</td><td bgcolor='#CCFFFF'>经手人</td>
  ...
  int curpage=1;//当前页
				...
				String fysql="select count(id) as ss from caigoujilu";
...
if(((curpage-1)*page_record)==0 || curpage==0)
  ...
  sql="select  * from caigoujilu where 1=1 ";
   ...
   {
   String tempsql="select  id from caigoujilu order by id desc limit 0,"+(curpage-1)*page_record+"";
  ...
  sql="select  * from caigoujilu where id not in ("+tempidstring+")  ";
  }
...
%>
  <tr>
    ...
    <td><%=bianhao %></td><td><%=mingcheng %></td><td><%=leibie %></td><td><%=kucun %></td><td><%=caigoushuliang %></td><td><%=caigoujia %></td><td><%=gongyingshang %></td><td><%=jingshouren %></td>
    <td width="138" align="center"><%=addtime %></td>
    <td width="60" align="center"><a href="caigoutuihuo_add.jsp?id=<%=id%>">退货</a> </td>
  </tr>
  	<%
  }
   %>
...
<p align="center">&nbsp;共<%=zgs%>条记录&nbsp;&nbsp;<%=page_record %>条/页　<a href="caigoujilu_list3.jsp?page=1">首页</a>　<a href="caigoujilu_list3.jsp?page=<%= curpage-1%>">上一页</a>　<A href="caigoujilu_list3.jsp?page=<%= curpage+1%>">下一页</A>　<a href="caigoujilu_list3.jsp?page=<%=zys %>">尾页</A>　当前第<FONT color=red><%=curpage %></FONT>页/共<FONT color=red><%=zys %></FONT>页</p>
  </body>
</html>
