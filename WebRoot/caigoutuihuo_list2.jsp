...
<html>
  <head>
    ...
    <title>进货退货</title><LINK href="CSS.css" type=text/css 
rel=stylesheet>
...
%>
  ...
  <form name="form1" id="form1" method="post" action="">
   ...
   <input name="leibie" type="text" id="leibie" style='border:solid 1px #000000; color:#666666' />
...
<table width="100%" border="1" align="center" cellpadding="3" cellspacing="1" bordercolor="00FFFF" style="border-collapse:collapse">  
  <tr>
    ...
    <td bgcolor='#CCFFFF'>进货数量</td><td bgcolor='#CCFFFF'>供应商</td><td bgcolor='#CCFFFF'>退货数量</td><td bgcolor='#CCFFFF'>退货事由</td><td bgcolor='#CCFFFF'>经手人</td>
  ...
  int curpage=1;//当前页
				...
				String fysql="select count(id) as ss from caigoutuihuo where jingshouren='"+request.getSession().getAttribute("username")+"'";
...
if(((curpage-1)*page_record)==0 || curpage==0)
   {
  sql="select  * from caigoutuihuo where jingshouren='"+request.getSession().getAttribute("username")+"' ";
   }
   else
   {
   String tempsql="select  id from caigoutuihuo where jingshouren='"+request.getSession().getAttribute("username")+"' order by id desc limit 0,"+(curpage-1)*page_record+"";
  ...
  sql="select  * from caigoutuihuo where id not in ("+tempidstring+") and jingshouren='"+request.getSession().getAttribute("username")+"' ";
  }
...
%>
  <tr>
    ...
    <td><%=bianhao %></td><td><%=mingcheng %></td><td><%=leibie %></td><td><%=caigoushuliang %></td><td><%=gongyingshang %></td><td><%=tuihuoshuliang %></td><td><%=tuihuoshiyou %></td><td><%=jingshouren %></td>
...
<p align="center">&nbsp;共<%=zgs%>条记录&nbsp;&nbsp;<%=page_record %>条/页　<a href="caigoutuihuo_list2.jsp?page=1">首页</a>　<a href="caigoutuihuo_list2.jsp?page=<%= curpage-1%>">上一页</a>　<A href="caigoutuihuo_list2.jsp?page=<%= curpage+1%>">下一页</A>　<a href="caigoutuihuo_list2.jsp?page=<%=zys %>">尾页</A>　当前第<FONT color=red><%=curpage %></FONT>页/共<FONT color=red><%=zys %></FONT>页</p>
  </body>
</html>
